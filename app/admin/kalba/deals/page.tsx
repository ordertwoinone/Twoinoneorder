...
const COLOR_PRESETS = [
  { bg: "#ecfdf5", day: "#16a34a", label: "Green"  },
  { bg: "#fefce8", day: "#ca8a04", label: "Yellow" },
  { bg: "#fdf2f8", day: "#db2777", label: "Pink"   },
  { bg: "#eff6ff", day: "#2563eb", label: "Blue"   },
  { bg: "#faf5ff", day: "#9333ea", label: "Purple" },
  { bg: "#fff7ed", day: "#ea580c", label: "Orange" },
];
...
export default function KalbaDealsAdmin() {
  const [items, setItems] = useState<DailyDeal[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<{ open: boolean; mode: "add" | "edit"; data: Omit<DailyDeal, "id"> & { id?: string } }>({
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/admin/kalba/deals");
  ...
  function openEdit(item: DailyDeal) { setModal({ open: true, mode: "edit", data: { ...item } }); }
  ...
  async function handleSave() {
    setSaving(true);
    if (modal.mode === "add") {
      await fetch("/api/admin/kalba/deals", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(modal.data),
      });
    } else {
      await fetch(`/api/admin/kalba/deals/${modal.data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(modal.data),
      });
  ...
  async function handleDelete() {
    if (!deleteId) return;
    await fetch(`/api/admin/kalba/deals/${deleteId}`, { method: "DELETE" });
    setDeleteId(null);
    load();
  }
    ...
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{items.length} deal{items.length !== 1 ? "s" : ""} · colored day cards</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Day</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center py-16 text-gray-400 text-sm">No deals yet.</td></tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  <span className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: item.bg_color }}>{item.emoji}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-[11px] font-bold uppercase" style={{ color: item.day_color }}>{item.day}</span>
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-base font-semibold text-gray-900">{modal.mode === "add" ? "Add deal" : "Edit deal"}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={modal.data.day} onChange={(e) => handleField("day", e.target.value)} className={`${inputCls} bg-white`}>
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {COLOR_PRESETS.map((preset) => {
                    const selected = modal.data.bg_color === preset.bg;
                    return (
                      <button key={preset.label} type="button"
                        onClick={() => { handleField("bg_color", preset.bg); handleField("day_color", preset.day); }}
                        className={`px-3 py-2 rounded-lg text-[11px] font-bold border-2 transition-colors ${selected ? "border-orange-400" : "border-transparent"}`}
                        style={{ background: preset.bg, color: preset.day }}>
                        {preset.label}
                      </button>
                    );
                  })}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 sticky bottom-0 bg-white rounded-b-2xl">
              ...
              <button onClick={handleSave} disabled={saving} className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "#ea580c" }}>
                {saving ? "Saving..." : modal.mode === "add" ? "Add deal" : "Save changes"}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-base font-semibold text-gray-900 text-center mb-2">Delete deal?</h3>
