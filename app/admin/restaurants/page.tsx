...
export default function RestaurantsAdmin() {
  ...
  async function handleSave() {
    ...
    if (modal.mode === "add") {
      ...
      await fetch(`/api/admin/restaurants/${modal.data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="text-center py-16 text-gray-400 text-sm">
                  {search ? "No results found." : "No restaurants yet. Add your first one."}
                </td>
              </tr>
            ) : (
              filtered.map((r) => (
                <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {r.badge ? (
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${BADGE_COLOR[r.badge] || "bg-gray-100 text-gray-600"}`}>
                        {r.badge}
                      </span>
                    ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-base font-semibold text-gray-900">
                {modal.mode === "add" ? "Add restaurant" : "Edit restaurant"}
              </h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (modal.mode === "add") handleField("slug", slugify(e.target.value));
                  }}
              ...
              {/* Food Image URL */}
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1.5">Food Image URL</label>
                <input
                  type="url"
                  value={modal.data.food_image_url}
                  onChange={(e) => handleField("food_image_url", e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                  placeholder="https://..."
                />
              </div>
