...
const ICON_OPTIONS = [
  ...
  { value: "Leaf",            label: "Leaf / Veg"       },
  { value: "Users",           label: "Users"            },
  { value: "ShieldCheck",     label: "Shield / Safe"    },
...
export default function WhyChooseUsAdmin() {
  ...
  const [modal, setModal] = useState<{ open: boolean; mode: "add" | "edit"; data: Omit<WhyChooseItem, "id"> & { id?: string } }>({
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/admin/buffet/why-choose-us");
  ...
  function openEdit(item: WhyChooseItem) { setModal({ open: true, mode: "edit", data: { ...item } }); }
  ...
  async function handleSave() {
    setSaving(true);
    if (modal.mode === "add") {
      await fetch("/api/admin/buffet/why-choose-us", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(modal.data),
      });
    } else {
      await fetch(`/api/admin/buffet/why-choose-us/${modal.data.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(modal.data),
      });
  ...
  async function handleDelete() {
    if (!deleteId) return;
    await fetch(`/api/admin/buffet/why-choose-us/${deleteId}`, { method: "DELETE" });
    setDeleteId(null);
    load();
  }
    ...
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{items.length} feature{items.length !== 1 ? "s" : ""} · shown as icon grid</p>
        </div>
        <button
          onClick={openAdd}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Sub Label</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="text-center py-16 text-gray-400 text-sm">No features yet.</td></tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  <div className="w-9 h-9 rounded-full bg-orange-50 flex items-center justify-center text-[10px] font-bold text-orange-600">
                    {item.icon_name.slice(0, 2)}
                  </div>
                </td>
                <td className="px-4 py-3 font-semibold text-gray-800">{item.label}</td>
                <td className="px-4 py-3 text-gray-500">{item.sub_label}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200">
              ...
              <button onClick={handleSave} disabled={saving} className="px-5 py-2.5 rounded-lg text-sm font-semibold text-white disabled:opacity-50" style={{ background: "#ea580c" }}>
                {saving ? "Saving..." : modal.mode === "add" ? "Add feature" : "Save changes"}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-base font-semibold text-gray-900 text-center mb-2">Delete feature?</h3>
