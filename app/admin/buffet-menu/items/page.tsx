...
import { Plus, Pencil, Trash2, X, Leaf, Star } from "lucide-react";
...
export default function MenuItemsAdmin() {
  ...
  const [modal, setModal] = useState<{ open: boolean; mode: "add" | "edit"; data: Omit<MenuItem, "id" | "buffet_menu_sections"> & { id?: string } }>({
  ...
  async function load() {
    ...
    const [itemsData, sectionsData] = await Promise.all([itemsRes.json(), sectionsRes.json()]);
  ...
  function openAdd() {
    setModal({
      open: true, mode: "add",
      data: { ...EMPTY, section_id: sections[0]?.id ?? "" },
    });
  }
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={modal.data.is_veg} onChange={(e) => handleField("is_veg", e.target.checked)}
                    className="w-4 h-4 rounded accent-green-600" />
                  <span className="text-sm font-medium text-gray-700">Vegetarian</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={modal.data.is_special} onChange={(e) => handleField("is_special", e.target.checked)}
