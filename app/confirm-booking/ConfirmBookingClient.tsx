...
// Update this to the company WhatsApp number (without + or spaces)
const COMPANY_WHATSAPP = '971501234567'
...
function buildWhatsAppText(b: PendingBooking) {
  const lines = [
    '🍽️ *New Table Booking - Two In One*',
    '',
...
export default function ConfirmBookingClient() {
  ...
  async function handleConfirm() {
    ...
    const res = await fetch('/api/bookings', {
      ...
      body: JSON.stringify({ ...booking, status: 'pending' }),
    })
  ...
  if (confirmed) {
    const waUrl = `https://wa.me/${COMPANY_WHATSAPP}?text=${buildWhatsAppText(booking)}`
    return (
      <div className="min-h-screen bg-[#F5F0E8] flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl shadow-sm max-w-md w-full p-8 text-center">
          ...
          <h2 className="text-xl font-extrabold text-[#1A1A1A] mb-2">Booking Confirmed!</h2>
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-[#25D366] text-white font-bold text-sm mb-3 hover:bg-[#1ebe5d] transition"
          >
          ...
          <Link href="/" className="block w-full py-3 rounded-2xl border border-gray-200 text-[#374151] font-semibold text-sm hover:bg-gray-50 transition">
            Back to Home
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#F5F0E8] text-[#1A1A1A]" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <main className="max-w-lg mx-auto px-3 sm:px-4 pb-8">
        ...
        <div className="pt-4 pb-1 text-center">
          ...
          <p className="text-[12.5px] text-[#6B7280] mt-0.5">Review your booking details before confirming</p>
        </div>
        ...
        {error && (
          <div className="mt-3 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-[13px] text-red-600">
            {error}
          </div>
        )}
        ...
        <div className="mt-4 space-y-3">
          <button
            ...
            disabled={confirming}
            className="w-full py-4 rounded-2xl bg-[#E8521A] text-white font-extrabold text-base hover:bg-[#F97316] transition shadow-md disabled:opacity-60"
          >
