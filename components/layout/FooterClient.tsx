...
export interface FooterContent {
  ...
  phone: string | null;
  email: string | null;
  socialLinks: { href: string; kind: SocialKind; label: string }[];
}

export type SocialKind = "facebook" | "instagram" | "twitter" | "tiktok" | "whatsapp";
...
const SOCIAL_ICONS: Record<SocialKind, () => JSX.Element> = {
...
export default function FooterClient({ content }: { content: FooterContent }) {
  ...
  const address = content.address
    ? `${content.address}${content.city ? `, ${content.city}` : ""}`
    : t("footer.defaultAddress");
  const phone = content.phone || "+971 52 230 5216";
  const email = content.email || "hello@twoinoneae.com";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-14">
          ...
          <div className="md:col-span-2 lg:col-span-1">
            ...
            <div className="flex gap-2 flex-wrap">
              {content.socialLinks.map(({ href, kind, label }) => {
          ...
          {/* Contact */}
          <div>
            <h4 className="text-gray-900 font-bold text-sm mb-4">{t("footer.getInTouch")}</h4>
            <ul className="space-y-3.5">
              ...
              <li>
                ...
                >
                  ...
                  <span className="force-ltr">{email}</span>
                </a>
              </li>
            </ul>
          </div>
          ...
          {/* Newsletter */}
          <div>
            <h4 className="text-gray-900 font-bold text-sm mb-4">{t("footer.stayInLoop")}</h4>
            <p className="text-[13px] text-gray-500 leading-relaxed mb-4 max-w-xs">
              {t("footer.newsletterText")}
            </p>
            <a
              href={`mailto:${email}?subject=${encodeURIComponent(t("footer.subscribeSubject"))}`}
              className="flex items-center justify-between gap-2 border border-gray-200 rounded-full ps-4 pe-1.5 py-1.5 max-w-xs hover:border-orange-300 transition-colors group"
            >
              <span className="text-[13px] text-gray-400">{t("footer.emailPlaceholder")}</span>
